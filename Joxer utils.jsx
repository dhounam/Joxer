...
// FILL ELEMENT
// See strokeElement, below
function fillElement(myE, cName) {
  var myCol;
  if (!g_colourSpaceRgb) {
    myCol = makeCmykColourObject(cName);
  }
...
function strokeElement(myE, cName, sWid) {
  ...
  if (myE.typename == 'PathItem') {
    ...
    // (Allows for traces beyond defined range)
    // Or, if we're using RGB
    if (typeof myCol !== 'undefined') {
      myE.strokeColor = myCol;
    }
    var overprint = c_textOverprint.search(cName) >= 0;
    if (overprint && cName == 'black75') {
      alert('overprinting ' + cName)
    }
