...
// Called from makeText to set Proportional/OldStyle on numbers in
// title, subtitle or sub-subtitle.
// As of Apr'24, no longer called
...
// SET TABULAR NUMBERS IN TABLES
// Called from makeText to set number in tables in tabular lining
// (Code pretty much duplicates doProportionalText)
function setTabularNumbersInTables(tRange) {
  var result;
  var str = tRange.contents;
  var patt = /\d+/g;
  var hasNumbers = true;
  while (hasNumbers) {
      ...
      if (result === null) {
          ...
          var numberRange = tRange.characters[start]
          numberRange.length = numbers.length;
          numberRange.characterAttributes.figureStyle = FigureStyleType.TABULAR;
      }
  }
}
// SET TABULAR NUMBERS IN TABLES ends
...
function makeText(tObj) {
	try {
      ...
      // RGB or CMYK colour object was defined in caller:
    ...
    // Proportional text for numbers in title cluster
    // deleted Apr'24
    ...
    // Tabular lining for tables
    if (tObj.name === 'table-text' && c_convertTableNumbersToTabularLining) {
        setTabularNumbersInTables(myText);
    }
		return myText;
...
//				}
    ...
    if (g_colourSpaceRgb) {
      // RGB: extract from current text object, to
      // transfer to new element
      textProps.fill = tGrp.textFrames[0].textRange.characterAttributes.fillColor;
    } else {
      // CMYK: create new cmyk fillColor from name
...
function makeNewTextFrame(oFrame, newGroup) {
  ...
  var anchorY = oFrame.anchor[1];
    ...
    textProps.fillName = tProps.fill;
    if (g_colourSpaceRgb) {
      textProps.fill = oFrame.textRange.characterAttributes.fillColor;
    } else {
      textProps.fill = makeCmykColourObject(tProps.fill);
    }
